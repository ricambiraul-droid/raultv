...
async function loadTV(acc) {
  ...
  return dedupe('tv|' + acc.id, async () => {
    ...
    // Grupurile reale gasite la furnizor: din ele construim lista de tari din
    // manifest, ca sa nu ramana nicio tara pe dinafara si nici optiuni goale.
    const conta = new Map();
    for (const x of list) conta.set(x.tag, (conta.get(x.tag) || 0) + 1);
    const tags = [...conta.keys()].filter(Boolean).sort((a, b) => {
      const d = (order.has(a) ? order.get(a) : 999) - (order.has(b) ? order.get(b) : 999);
      return d || a.localeCompare(b);
    });
    const grupuri = tags.map(t => ({ tara: t, canale: conta.get(t) }));
    s.tv = { at: Date.now(), rows: list, byId: new Map(list.map(x => [x.id, x])), tags, grupuri };
...
// Tarile gasite efectiv la furnizor, pentru manifest si pentru diagnostic.
function tvTags(acc) {
  const s = stores.get(acc && acc.id);
  return (s && s.tv && s.tv.tags) || [];
}
function tvGroups(acc) {
  const s = stores.get(acc && acc.id);
  return (s && s.tv && s.tv.grupuri) || [];
}
// Cat timp lista nu e incarcata, lista statica ramane plasa de siguranta.
function tvGenres(acc) {
  const t = tvTags(acc);
  if (!t.length) return TV_GENRES;
  const deBaza = ['RO', 'IT'].filter(g => !t.includes(g));
  return [...t, ...deBaza];
}
